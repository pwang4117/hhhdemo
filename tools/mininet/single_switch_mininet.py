...
parser.add_argument('--cli', help='Path to switch CLI',
...
parser.add_argument('--message', help='Message to print before starting CLI',
                    type=str, action="store", required=False, default=None)
...
def main():
    ...
    sw_mac = ["00:aa:bb:00:00:%02x" % n for n in xrange(1, num_hosts+1)]

    sw_addr = ["10.0.%d.1" % n for n in xrange(1, num_hosts+1)]
    ...
    if args.switch_config is not None:
        ...
        proc = Popen([args.cli, "--pre", "SimplePreLAG"], stdin=PIPE)
    ...
    if args.message is not None:
        with open(args.message, 'r') as message_file:
